...
\texttt{record.py} 的 \texttt{record\_loop()} 到 \texttt{scservo\_sdk} 的
完整 Python 软件栈。
...
\begin{figure}[htbp]
  ...
  \caption{LeRobot 执行阶段 Python 软件栈调用链}
...
\subsubsection{关键性能结论}\label{sec:execution-conclusion}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{瓶颈在物理层而非软件路径}：1 Mbps 波特率下，
        Sync Read 帧 14--15 字节、Sync Write 帧约 26 字节（6 ID × 4 字节位置 + 包头 + 校验），
        总线传输时间是耗时主体；
