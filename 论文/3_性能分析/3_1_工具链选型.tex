...
本节先比较 \texttt{strace} 和 \texttt{ftrace} 两类常用工具，再用同一条推理循环实测它们带来的扰动，
最后确定后续章节采用的内核态追踪方案。
这种先校准测量工具、再解释系统瓶颈的做法，也符合系统性能分析中强调的
``先确认观测开销，再分析被测系统''的基本原则\cite{perfbook}。

\texttt{strace}\cite{strace} 的优点是使用方便，能够直接看到进程发起了哪些系统调用。
但它的工作方式决定了开销较大：目标进程每进入或退出一次系统调用，
都会被 \texttt{ptrace} 暂停，并切换到 \texttt{strace} 进程读取参数和返回值。
在摄像头读取、串口通信和线程同步都很频繁的在线推理循环中，
这种逐次拦截会放大上下文切换次数，进而改变原本的帧率和阶段耗时结构。
因此，\texttt{strace} 更适合用来定性确认调用类型，而不适合作为正式的定量时序数据来源。

\texttt{ftrace}\cite{ftrace} 的记录位置更靠近内核。
系统调用、调度或自定义事件触发后，内核直接把记录写入内部缓冲区，
不需要为每一次事件切换到单独的跟踪进程，所以插桩本身对主循环的干扰更小。
对本文而言，\texttt{ftrace} 的另一个关键优点是可以把标准内核事件和自定义内核日志放在同一条追踪流中。
例如，系统调用事件负责记录 \texttt{sys\_pselect6}、\texttt{sys\_futex} 的进入和退出，
自编译内核中的 \texttt{trace\_printk} 则负责输出 \texttt{do\_select()} 和
\texttt{do\_futex()} 内部的总耗时、睡眠时间和 CPU 时间等细分信息。
这些记录最终共享同一个 \texttt{ftrace} 缓冲区、同一套时间戳、CPU 编号和进程信息，
因此可以直接把 Python 主循环、摄像头等待和 futex 线程同步放到同一条时间线上分析，
不需要再在用户态日志和内核日志之间额外做时钟校准。
不过，\texttt{ftrace} 也不是完全无代价：采集结束后导出 \texttt{trace} 文件，
或在采集过程中持续读取 \texttt{trace\_pipe}，都会产生额外的文件读写、CPU 占用和内存带宽消耗；
当事件过于密集时，缓冲区还可能覆盖旧记录。
因此，本文不仅比较 \texttt{strace} 和 \texttt{ftrace}，
也进一步区分 \texttt{ftrace} 的延迟导出和实时导出两种使用方式。

为定量比较这些工具对主循环的影响，本文设计了测量工具校准实验。
实验固定动作块长度为 100、目标帧率为 30\,fps，并在约 30\,s 的同一类推理循环上比较四组配置。
A 组为基准组，不开启任何追踪；B 组使用 \texttt{strace -f -ttT} 包裹运行命令；
C 组只让 \texttt{ftrace} 写入内核缓冲区，待单段数据采集结束后再导出文件；
D 组同样开启 \texttt{ftrace}，但同时启动后台进程持续读取 \texttt{trace\_pipe} 并写入文件。
帧率、上下文切换次数和阶段时间占比由 \texttt{record.py} 通过
\texttt{resource.getrusage()} 与 sysfs 自动采集，并写入
\texttt{analysis/timing\_stats.csv}。表~\ref{tab:tool-overhead} 汇总了四组的核心结果。
...
这种数量级上的放大正是 \texttt{ptrace} 机制的直接结果：
摄像头链路中密集出现的 \texttt{pselect6}、\texttt{read}、\texttt{ioctl} 等系统调用，
都会被 \texttt{strace} 反复打断和恢复。
相比之下，C 组的上下文切换次数只比 A 组略高，
D 组的额外切换主要来自后台读取 \texttt{trace\_pipe} 的进程。
这说明 \texttt{ftrace} 写入内核缓冲区这一步本身并不会显著扰动调度路径。
...
说明 \texttt{strace} 把摄像头读取等系统调用密集的部分严重拉长，
并挤压了原本用于补足帧间隔的等待阶段。
这种结构性扭曲比单纯的帧率下降更严重，
...
\texttt{ftrace} 延迟导出作为主要内核态时序采集方式。
...
\texttt{strace} 不参与最终定量分析，只在早期用于确认系统调用类型分布。

标准 \texttt{ftrace} 事件只能告诉本文某个系统调用何时进入、何时退出以及返回值是什么，
但无法继续拆分系统调用内部到底是在睡眠等待，还是在 CPU 上处理数据。
为了获得更细的时间分解，本文在自编译内核中加入了少量源码级插桩：
通过 \texttt{trace\_printk} 将自定义计时信息写入同一个 \texttt{ftrace} 缓冲区，
再与标准事件一起导出分析。

第一处插桩位于 \texttt{pselect6} 的内核实现路径，
具体位置是 \texttt{fs/select.c} 中的 \texttt{do\_select()}。
该函数是摄像头等待帧就绪时的重要入口。
本文在函数入口记录起始时间，在 \texttt{poll\_schedule\_timeout()} 前后记录阻塞睡眠时间，
并在函数退出时输出总耗时、睡眠时间、上下文处理时间、返回值、就绪文件描述符以及设备路径。
这样一来，追踪结果中可以直接区分
``\texttt{fd=8, file=video0}'' 对应的手眼摄像头和
``\texttt{fd=9, file=video2}'' 对应的固定摄像头。
实测中，每次摄像头等待约为 30\,ms，其中睡眠时间约占总耗时的 99.7\%，
说明该路径主要是在等待新帧，而不是在 CPU 上持续计算。

第二处插桩位于 \texttt{kernel/futex/syscalls.c} 中的 \texttt{do\_futex()}。
该路径与 Python 运行时和 PyTorch 线程同步有关\cite{pytorch}。
本文在函数入口以及 \texttt{futex\_wait\_multiple()} 前后打点，
...
相比之下，在 \texttt{do\_select()} 的原生执行上下文中调用 \texttt{d\_path()} 更可控。
