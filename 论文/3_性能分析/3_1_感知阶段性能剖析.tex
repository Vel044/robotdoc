...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=blue!60, fill=blue!7, rounded corners=2pt,
      minimum width=2.20cm, minimum height=0.70cm, align=center, font=\scriptsize},
    cvbox/.style={rectangle, draw=cyan!60!black, fill=cyan!8, rounded corners=2pt,
      minimum width=2.20cm, minimum height=0.70cm, align=center, font=\scriptsize},
    ...
    \node[layer] at (-1.55, 4.75) {LeRobot};
    \node[box] (loop) at (0.95, 4.75) {后台读线程};
    \node[box] (camread) at (3.45, 4.75) {相机 read};

    \draw[dashed, gray!35] (-0.20, 4.05) -- (8.65, 4.05);
    \node[layer] at (-1.55, 3.25) {OpenCV};
    \node[cvbox] (vcap) at (2.00, 3.25) {VideoCapture\\read};
    \node[cvbox] (grab) at (4.70, 3.25) {grab\\尝试 ioctl};
    \node[cvbox] (retrieve) at (7.35, 3.25) {retrieve\\MJPEG 解码};

    \draw[dashed, gray!35] (-0.20, 2.48) -- (8.65, 2.48);
    \node[layer] at (-1.55, 1.55) {glibc / syscall};
    \node[waitbox] (select) at (2.00, 1.55) {select\\等待 fd 可读};
    \node[waitbox] (ioctl) at (4.70, 1.55) {ioctl\\VIDIOC\_DQBUF};
    \node[waitbox] (pselect) at (2.00, 0.45) {pselect6\\睡眠等待};

    \draw[dashed, gray!45] (-0.20, -0.20) -- (8.65, -0.20);
    \node[note] at (7.75, -0.02) {系统调用边界};

    \node[layer] at (-1.55, -1.10) {Linux kernel};
    \node[kbox] (v4l2) at (2.00, -1.10) {V4L2 / UVC\\设备驱动};
    \node[kbox] (vb2) at (4.70, -1.10) {videobuf2\\完成队列};
    \node[kbox] (wq) at (2.00, -2.18) {done\_wq\\等待队列};

    \draw[->, thick, blue!65] (loop) -- (camread);
    \draw[->, thick, blue!65, rounded corners=5pt]
      (camread.south) -- ++(0,-0.35) -| (vcap.north);
    \draw[->, thick, cyan!60!black] (vcap) -- (grab);
    \draw[->, thick, cyan!60!black] (grab) -- (retrieve);

    \draw[->, thick, green!55!black] (grab.south) -- node[pos=0.30, right, note]{已有完成帧} (ioctl.north);
    \draw[->, thick, green!55!black] (ioctl) -- (vb2);

    \draw[->, thick, orange!75]
      (grab.south west) -- node[pos=0.52, left, note]{EAGAIN 时调用 select} (select.north east);
    \draw[->, thick, orange!75] (select) -- (pselect);
    \draw[->, thick, orange!75] (pselect) -- (v4l2);
    \draw[->, thick, orange!75] (v4l2) -- node[right, note]{新帧完成} (wq);
    \node[note] at (2.00, -2.85) {唤醒后重试 ioctl};
  \end{tikzpicture}
