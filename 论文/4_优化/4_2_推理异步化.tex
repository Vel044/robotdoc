\subsection{推理异步化}\label{sec:async-inference}

\subsubsection{设计思路}\label{sec:async-inf-motivation}
...
\subsubsection{实现结构}\label{sec:async-inf-arch}

如图~\ref{fig:async-inf-arch}所示，LeRobot 将控制与推理拆分为两个独立进程，
通过 gRPC（远程过程调用框架）交换观测与动作序列。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{4_优化/images/async_inference_arch.pdf}
  \caption{异步推理架构：Robot Client 与 Policy Server 是两个独立进程。前者负责硬件连接、观测采集、控制循环与动作执行，后者负责加载模型、完成推理并返回动作序列；二者通过 gRPC 交换带时间戳和步号的观测与动作序列。}
  \label{fig:async-inf-arch}
\end{figure}

其中，\texttt{robot\_client.py} 进程负责连接相机和舵机、维持控制循环，并将当前观测发送给
\texttt{policy\_server.py} 进程；
\texttt{policy\_server.py} 进程负责加载 ACT 模型，在收到观测后完成一次动作序列预测，
再将结果返回给 \texttt{robot\_client.py} 进程。这样，动作执行仍由本地控制循环逐帧推进，
...
\textbf{第三是空队列兜底}：
若推理速度不足，导致动作队列接近耗尽，\texttt{robot\_client.py} 进程会附带发送一个强制触发标记，
要求 \texttt{policy\_server.py} 进程立即处理当前观测，而不再等待常规去重或合并逻辑。
这条路径本质上是异常兜底，用来避免队列彻底清空后控制循环停顿。
因此，强制触发出现得越频繁，越说明当前异步流水线离稳定工作区越远，
它也就成为后续实验中判断系统健康度的重要观测量。

\subsubsection{可行性不等式与 hold 参数}\label{sec:async-inf-inequality}

异步推理的稳态行为受四个参数控制：chunk\_size $N$、
单次推理耗时 $T_{\text{infer}}$、目标执行频率 $F$、
以及提前触发阈值 $h$（即 \texttt{chunk\_size\_threshold}）。
在 LeRobot 当前实现中，触发条件为
\texttt{action\_queue.qsize() / action\_chunk\_size <= h}，
因此 \textbf{$h$ 越大，触发越早；$h$ 越小，触发越晚}。
...
\texttt{timestep <= latest\_action} 带来的 1 帧端点差异。

代码中存在两个不同层次的约束。第一，触发必须足够早：
当队列剩余 $hN$ 帧时发出观测，如果 $D>hN$，
新动作在当前队列耗尽前无法返回，控制循环必然出现 stall。因此有
\begin{equation}
  D \leq hN
...
\texttt{policy\_server.py} 返回的动作步号从观测步号开始递增，
而 \texttt{robot\_client.py} 会丢弃所有
\texttt{timestep <= latest\_action} 的动作。
若返回前没有 stall，推理期间已经执行了约 $D$ 帧，
因此新 chunk 中真正能进入未来队列的帧数约为
\begin{equation}
...
每周期产生约 20 帧 stall；stall 期间 \texttt{latest\_action} 冻结在第 99 帧，
新 chunk 过期帧为 $[50,99]$ 共 50 帧（非物理意义上的 70 帧），
有效帧 $[100,149]$ 共 50 帧进入队列，稳态有效 FPS $\approx 50/2.33 = 21.4$。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../推理相关/async_inference_A.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../推理相关/async_inference_B.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{../推理相关/async_inference_C.png}
...
\texttt{latest\_action} 不再随物理时间前进，所以返回 chunk 的过期部分
不会继续扩大。对默认 $h=0.5$ 而言，进入周期性 stall 后，
每轮返回大约仍有 $N/2=50$ 帧可执行；理想化有效 FPS 近似为：
\begin{equation}
...
从 hold 扫描的角度看，更关键的是找到实际临界点：
当 hold 小于临界值时，触发太晚，stall 周期性出现；
当 hold 越过临界值后，新 chunk 能在旧队列耗尽前到达，
理想稳态就应恢复到 30 FPS。
因此问题从``异步推理是否可用''进一步细化为：
\textbf{在动作质量仍可接受的前提下，实际临界 hold 在哪里，
以及推理本体还需降到多快才能给该临界点留下足够余量}。
4.3 节即围绕该参数展开实验扫描。

需要指出的是，上述不等式是在``每次触发都会立即启动一次新推理''的理想假设下推导的。
在 LeRobot 的实际实现中，\texttt{policy\_server.py} 还会对观测做帧号去重和相似性过滤；
同时，\texttt{robot\_client}、\texttt{policy\_server}、相机线程和串口通信在树莓派 5
上竞争同一组 CPU 核心。因此，实验中真正起作用的不是纯粹的模型前向耗时，
而是从触发观测到新动作 chunk 合并进队列的有效返回时延。
下文把这些实现细节统一吸收到 $T_{\text{rt}}$ 中，
再用 4.3 节的实测数据确定实际临界点。

\subsubsection{FPS 与 hold 的定量关系}\label{sec:async-inf-fps-hold}

4.2.3 节的不等式只给出了稳定无 stall 的边界，却没有回答一个更具体的问题：
在到达该边界之前，也就是确定会发生 stall 的前段区间，
有效 FPS 应该如何随 hold 变化？
为避免把代码实现中的抖动误写成理论规律，本节只分析理想调度下的主导关系：
\textbf{临界点之前 FPS 随 hold 上升，临界点之后饱和到目标帧率 30 FPS}。
以下推导只针对临界点之前的 stall 段，即
$h<h_{\text{crit}}$ 且 $T_{\text{rt}}>hN/F$。
实验中的 $h=0.60$ 仅作为偏早触发的参考点，不纳入该段公式。

令 $T_{\text{rt}}$ 表示从 client 触发观测到新动作 chunk 合并进队列的有效返回时延。
