...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{2_实验环境构建/image/record_loop.pdf}
...
\textbf{执行阶段}将预测动作通过 scservo\_sdk 写回舵机，
路径与观测阶段的关节读取对称。\textbf{等待阶段}由 \texttt{busy\_wait()} 补足帧间隔；
在树莓派 Linux 环境下，该函数会调用 \texttt{time.sleep()}，
底层通常通过 \texttt{clock\_nanosleep} 进入内核定时睡眠路径，使进程主动让出 CPU。
...
最上层是由 \texttt{record.py} 组织的应用逻辑，它按照观测、推理、执行和等待的顺序推进每一帧；
