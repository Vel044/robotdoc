...
\subsection{性能记录与日志对齐方法}\label{sec:async-exp-metrics}

client 端记录的核心指标包括：执行帧数、stall 帧数、过期帧数、
must\_go 触发次数、帧率统计以及 CPU 温度与频率。
其中 $\mathrm{action\_frames}$ 表示 episode 内真正执行动作的帧数，
$\mathrm{episode\_total\_s}$ 表示 episode 的实际总持续时间，单位为秒；
有效 FPS 定义如式（\ref{eq:exp-effective-fps}）所示：
\begin{equation}
  F_{\text{eff}} = \frac{\mathrm{action\_frames}}{\mathrm{episode\_total\_s}}.
  \label{eq:exp-effective-fps}
\end{equation}
