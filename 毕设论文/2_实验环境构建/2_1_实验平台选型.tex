\section{实验平台选型}\label{sec:hw-platforms}

本文选用 LeRobot——HuggingFace 推出的开源机器人学习库，
...
相比之下，基于 ROS2 的方案以 DDS 为底层通信中间件，
单次跨节点消息传递引入 1–5\,ms 延迟，DDS 守护进程还额外占用 CPU 与内存；
...
SO-101 机械臂实物及关节编号如图~\ref{fig:so101-hardware}所示，
本文后续分析中使用的关节与摄像头编号见表~\ref{tab:so101-joint-camera}。

上述平台选型遵循三项原则。其一，部署一致性优先：本文关注的是树莓派5上在线控制回路的真实时延，
因此必须直接采用 ARM CPU 无 GPU 的目标硬件，而不能以桌面级 x86 或独立显卡平台替代。
其二，可剖析性优先：LeRobot 的单进程串行架构使观测、推理与执行三段边界清晰，
便于将耗时继续分解到 OpenCV/V4L2、PyTorch、pyserial 以及 Linux 内核调用路径。
其三，代表性与成本平衡：SO-101 与双相机配置同时覆盖视觉输入、神经网络前向计算和舵机总线控制三类典型负载，
能够在低成本具身智能终端上复现完整在线推理链路，为后续瓶颈定位与优化策略评估提供稳定实验基础。
...
\begin{table}[htbp]
  \centering
  \caption{SO-101 关节与摄像头编号}
  \label{tab:so101-joint-camera}
...
策略模型选用 ACT（Action Chunking with Transformers），
...
Diffusion Policy 参数量相近但扩散推理步数多导致延迟更高；
ACT 是本文研究场景下可在树莓派5 上实时运行的唯一可行选型。
实验平台的硬件与采集配置汇总见表~\ref{tab:experiment-hardware-config}。

\begin{table}[htbp]
  \centering
  \caption{实验平台硬件与采集配置}
  \label{tab:experiment-hardware-config}
  ...
  控制参数 & chunk\_size 为 100，控制目标 fps=30，串口波特率 1 Mbps \\
  采集设置 & 每类任务采集 $\geq 3$ 次 episode 取均值，排除冷启动影响 \\
  \bottomrule
  \end{tabularx}
\end{table}

为保证实验复现性，本文使用的软件环境与主要依赖版本如表~\ref{tab:experiment-software-config}所示。

\begin{table}[htbp]
  \centering
  \caption{实验平台软件环境}
  \label{tab:experiment-software-config}
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}X@{}}
  \toprule
  软件/库 & 版本或说明 \\
  \midrule
