...
由在线控制程序加载策略模型执行在线闭环推理，实验数据均在此阶段采集。

三类任务按操作复杂度从低到高设计，具体任务设置如表~\ref{tab:task-design}所示，
以考察不同难度下系统的推理行为是否存在差异。
...
三类任务统一使用树莓派5、ACT 策略、chunk\_size 为 100、目标帧率 30\,fps，
...
更适合用来观察执行节奏和 chunk\_size 对整体帧率的影响。

至此，本文的实验对象已经确定为一条固定的在线推理链路：
树莓派5 在 ARM CPU 上执行 ACT 策略，两路 USB 摄像头提供视觉观测，
SO-101 follower 通过舵机总线完成动作执行。
三类任务的差异主要体现在目标数量、动作路径和等待节奏上，
而软件调用路径保持一致。
这一设置保证了后续第\ref{ch:perf}章可以在同一套阶段划分下比较观测、推理和执行开销，
并将阶段耗时追溯到 OpenCV/V4L2、PyTorch、pyserial 以及 Linux 内核路径。
