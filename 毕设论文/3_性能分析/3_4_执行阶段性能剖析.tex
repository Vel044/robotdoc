\section{执行阶段性能剖析}\label{sec:execution}
...
\subsection{执行路径的软件关系}\label{sec:execution-software-stack}
...
\subsection{写入链路：从目标位置到串口}\label{sec:sync-write-chain}
...
对应用程序而言，/dev/ttyACM0 像一个普通串口文件，写入动作就是向这个文件发送一段舵机协议字节。
...
\subsection{读取链路：从请求帧到逐电机回包}\label{sec:sync-read-chain}
...
关键性能结论\label{sec:execution-conclusion}：
